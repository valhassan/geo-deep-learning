"""SIGReg loss. Adapted from https://github.com/galilai-group/lejepa/tree/main ."""
...
class SIGReg(nn.Module):
    ...
    def forward(self, embeddings: torch.Tensor) -> torch.Tensor:
        """Run the SIGReg."""
        expected_ndims = (2, 3)
        if embeddings.ndim == expected_ndims[1]:
            # [V, B, D] -> [N, D]
            x = embeddings.reshape(-1, embeddings.size(-1))
        elif embeddings.ndim == expected_ndims[0]:
            x = embeddings
        else:
            # (*, N, D) keep as-is
            x = embeddings
